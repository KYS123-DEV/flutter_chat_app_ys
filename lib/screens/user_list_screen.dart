...
class _UserListScreenState extends State<UserListScreen> {
  ...
  Widget build(BuildContext context) {
    // 🚀 수정: Scaffold 전체를 감싸서 AppBar까지 상태에 따라 리빌드되도록 변경
            ...
            actions: [
              ...
              if (_viewModel.selectedUids.isNotEmpty)
                IconButton(
                  ...
                  onPressed: () {
                    _navigateToChat("그룹채팅 (${_viewModel.selectedUids.length + 1}명)");
                  },
                ),
            ],
          ),
            ...
            builder: (context, snapshot) {
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  if (userUid == _viewModel.currentUid) return const SizedBox.shrink();

                  return CheckboxListTile(
                    value: _viewModel.selectedUids.contains(userUid),
                    secondary: CircleAvatar(
                      child: profileUrl.isNotEmpty ? Image.network(profileUrl) : const Icon(Icons.person),
                    ),
                    title: Text(userName),
